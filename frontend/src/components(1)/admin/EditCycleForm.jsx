...
import { BRANDS, CATEGORIES, FRAME_MATERIALS, SUSPENSION_TYPES, SKILL_LEVELS } from '../../utils/constants';
...
const SharpSelect = ({ label, register, name, options, required }) => (
    <div className="mb-4">
        <label className="block font-body text-[10px] uppercase text-text-secondary tracking-[0.2em] mb-2">{label}</label>
        <select
...
export default function EditCycleForm({ rowId, initialData, onSubmit, onClose }) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-4xl bg-bg-dark border border-border-mid flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-border-subtle flex justify-between items-center sticky top-0 bg-bg-dark z-10">
                    <h2 className="font-heading text-3xl text-gold uppercase tracking-widest m-0">Edit Telemetry: {initialData?.name}</h2>
                    <button onClick={onClose} className="text-text-muted hover:text-gold transition-colors focus:outline-none">✕</button>
                </div>

                <form onSubmit={handleSubmit((data) => onSubmit(rowId, data))} className="overflow-y-auto p-6 space-y-8 custom-scroll">
                    ...
                    <div className="sticky bottom-0 bg-bg-dark border-t border-border-subtle pt-4 pb-2 flex justify-end gap-4 mt-8">
                        ...
                        <button type="submit" disabled={isSubmitting} className="px-8 py-2 bg-gold text-bg-deepest font-heading text-lg uppercase tracking-widest hover:bg-text-primary transition-colors focus:outline-none">
